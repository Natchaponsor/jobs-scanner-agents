...
describe("filterAndSortJobs — work authorization filter", () => {
  it("passes every job through when the filter is 'any'", () => {
    const jobs = [
      makeJob({ id: "1", workAuthorization: "Citizenship Required" }),
  ...
  it("restricts US jobs to the selected work authorization value", () => {
    const jobs = [
      makeJob({ id: "us-citizen", locationCountry: "United States", workAuthorization: "Citizenship Required" }),
    ...
    const result = filterAndSortJobs(jobs, { ...baseFilters, workAuthorization: "Citizenship Required" }, opts);
  ...
  it("[feature] never excludes non-US jobs, regardless of the selected value — the filter is US-only by design", () => {
    ...
    // "Citizenship Required" — only the US posting is actually held to the filter, and it's
  ...
  it("combines with other filters normally (e.g. explicitly scoping to United States)", () => {
    const jobs = [
      ...
      makeJob({ id: "us-miss", locationCountry: "United States", workAuthorization: "Citizenship Required" }),
